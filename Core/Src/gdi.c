...
void draw_picture(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t *picture, gdi_handle *handle) {
...
void draw_picture_blend(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t *picture, gdi_handle *handle) {
    if(x + width >= handle->width || y + height >= handle->height) {
        return;
    }
